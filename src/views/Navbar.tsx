...
function scrollToId(id: string, bannerVisible: boolean) {
  ...
  // NOTE: main's version routed this through Lenis (window.__lenis). The Next app has
  // no Lenis - the migration dropped the dependency - so this uses the native smooth
  // scroll. Restoring Lenis is tracked separately; it is a real behavioural regression
  // that the pixel gate cannot see.
...
export default function Navbar({ transparent = false }: NavbarProps) {
  ...
  // Starts false on both server and client, then reads the stored dismissal after
  // mount. main read localStorage inside useState's initialiser, which was fine for a
  // client-only Vite app but would hydrate mismatched here: the server always renders
  // "not dismissed" and a returning visitor's client would disagree on first paint.
  const [bannerDismissed, setBannerDismissed] = useState(false);
  useEffect(() => {
    try {
      if (localStorage.getItem(BANNER_STORAGE_KEY) === '1') setBannerDismissed(true);
    } catch {
      /* localStorage unavailable (private mode); showing the banner is the safe default */
    }
  }, []);
